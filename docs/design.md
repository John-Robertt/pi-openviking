...
本文只规定稳定边界：模块之间交付什么结果、遵守什么契约、依赖朝哪个方向。

数据从哪里来、以哪份数据为准，以及如何存储、索引、同步、生成和缓存，由负责这项结果的模块决定，写在该模块的架构设计里；外部 SDK、配置和凭证同样属于对应模块。因此查一项机制怎样工作，去负责它的模块设计；查模块之间的约定，来本文。
...
调用方给出线索数量和展示字符预算，Cue Provider 返回的 `CueSet` 必须落在预算内。线索如何生成、保存和更新，由 Cue Provider 的模块设计负责。
