...
**架构定位**：证明系统满足 [`docs/v1/spec.md`](./spec.md) 所需证据的标准。

**核心目标**：任何验证工作据此判断“什么证据算数”——证据的种类、live verifier 必须遵守的契约，以及
各阶段门禁必须由机器断言的结果。

**职责边界**：本文只定义证据标准与门禁契约，不含阶段进度和下一步——那些由
[`docs/v1/roadmap.md`](./roadmap.md) 维护。观察记录的形状与脱敏由
[`docs/v1/observability.md`](./observability.md) 定义，本文只消费其产出。
...
划定事件范围的上下文权重是确定性策略量，由 `docs/v1/spec.md` 定义，不属于该证据类别。模型输出允许非确定，
...
观察记录支持上述四类证据，但不构成新的产品事实。其完整性与安全性按 `docs/v1/observability.md` 判定，验证方法
固定如下：
