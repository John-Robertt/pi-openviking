 ...
 * OpenViking wheel. Decision-free mechanisms parameterized by home, reporting
 * through log and throwing instead of exiting: prompts, config parsing and
 * lifecycle stay in the calling script.
 */
...
//（同时遵循 NO_PROXY）。注意 setGlobalDispatcher 影响进程内全部后续 fetch；调用方在下载之外
// 新增 fetch 调用时需重新评估此前提。undici 随 Pi 依赖进入，导入失败时静默退回直连。
...
export function ensurePython({ home, log = () => {} }) {
  ...
  if (!runProcess(paths.uvBin, ["python", "install", "--no-bin", TOOLCHAIN.pythonVersion], { env: uvEnv(paths) }).ok) {
    throw new Error("托管 Python 安装失败。网络受限时可设置 UV_PYTHON_INSTALL_MIRROR 后重试。");
  }
  return paths;
}
