...
function normalizeRole(role) {
  ...
  // pi names these roles in camelCase; "toolcall" was already handled here but
  // "toolresult" was not, so every tool result was dropped before capture and
  // never reached OpenViking.
  if (value === "tool" || value === "tool_result" || value === "toolresult") return "user";
  if (value === "tool_call" || value === "toolcall") return "assistant";
  return "";
}

function messageKind(payload) {
  const value = String(payload?.role || payload?.type || payload?.kind || "").toLowerCase();
  if (value === "user") return "user_query";
  if (value === "assistant" || value === "tool_call" || value === "toolcall") return "assistant_step";
  if (["tool", "tool_result", "toolresult", "function_call_output"].includes(value)) return "tool_transport";
  return "";
}
...
export function extractBranchCapturePayloads(branch, syncedEntryCount = 0, cfg = {}) {
  ...
  for (const entry of entries.slice(0, start)) {
    ...
    if (messageKind(payload) === "user_query") currentTurnId = String(entry?.id || "");
  }
  ...
  for (const entry of entries.slice(start)) {
    ...
    if (kind === "user_query") currentTurnId = String(entry?.id || currentTurnId);

    const role = normalizeRole(payload.role || payload.type || payload.kind);
    if (!role) continue;
    if (role === "assistant" && cfg.captureAssistantTurns === false && !cfg.takeoverEnabled) continue;
    ...
    if (kind) body.message_kind = kind;
    if (currentTurnId) body.turn_id = currentTurnId;
