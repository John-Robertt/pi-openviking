...
export class TakeoverCore {
  ...
  get state() {
    return {
      ...
      currentTurnTokens: this.currentTurnTokens,
      currentTurnUserTurns: this.currentTurnUserTurns,
  ...
  restore(entries) {
    for (let i = (Array.isArray(entries) ? entries.length : 0) - 1; i >= 0; i--) {
      ...
      if (Number(data.schemaVersion) === 2) {
        ...
        this.currentTurnTokens = Math.max(0, Math.floor(Number(data.currentTurnTokens) || 0));
        this.currentTurnUserTurns = Math.max(0, Math.floor(Number(data.currentTurnUserTurns) || 0));
  ...
  transformContext(messages) {
    ...
    if (seenUserTurns !== this.currentTurnUserTurns) {
      this.currentTurnUserTurns = seenUserTurns;
      this.currentTurnTokens = 0;
    }
  ...
  async onTurnSynced(estTokens) {
    ...
    this.currentTurnTokens += addedTokens;
    if (this.currentTurnTokens > this.config.takeoverRetainedTokenBudget) {
  ...
  acceptCommit(committed, purpose) {
    ...
    if (committed.budget_exceeded === true) this.compactionRequested = true;
